...
EXPANDED_QUERIES = [
    ...
    "lovers", "hubby", "wifey", "family+time", "twin+mom", "twin+dad",
    "toddler+mom", "baby+boy", "baby+girl", "pregnancy", "newborn",
    # US States & Metros
    ...
    "memphis", "baltimore", "milwaukee", "albuquerque", "tucson", "sacramento",
    "kansas+city", "omaha", "colorado+springs", "raleigh", "virginia+beach", "minneapolis",
    "ohio", "michigan", "georgia", "north+carolina", "virginia", "washington",
    "arizona", "tennessee", "indiana", "missouri", "maryland", "wisconsin",
    "colorado", "minnesota", "south+carolina", "alabama", "louisiana", "kentucky",
    "oregon", "oklahoma", "connecticut", "utah", "iowa", "nevada", "arkansas",
    # Lifestyle & Content niches with couples/families
    "family+travel", "home+decor", "fashion+couple", "fitness+couple", "foodie+couple",
    "cooking+together", "our+home", "lifestyle+blogger", "southern+living", "midwest+life",
    "coastal+living", "mountain+life", "suburban+life", "city+life", "family+adventure",
    "roadtrip", "vanlife", "camping+family", "outdoor+family", "family+fitness"
]
...
COUPLE_KEYWORDS = [
    ...
    "relationship", "dating", "together", "family", "parents", "parenting",
    "mom & dad", "mom and dad", "him and her", "partners", "duo",
    "love", "traveling couple", "travel couple", "couple goals", "living together",
    "mr & mrs", "mr and mrs", "mom of", "dad of", "raising", "family life",
    "fiance", "fiancé", "our journey", "our story", "two of us", "boyfriend",
    "girlfriend", "mama", "papa", "twins", "toddler", "baby", "mom", "dad",
    "mother", "father", "family vlog", "wife & mom", "husband & dad",
    "family of", "travel family", "adventure couple", "boy mom", "girl mom",
    "twin mom", "twin dad", "motherhood", "fatherhood", "engaged", "lovers",
    "hubby", "wifey", "family time", "wedding"
]
...
def is_couple_creator(creator: dict) -> bool:
    bio = str(creator.get("biography", "")).lower()
    name = str(creator.get("name", "")).lower()
    uname = str(creator.get("username", "")).lower()
    cat = str(creator.get("category", "")).lower()
    q = str(creator.get("query_matched", "")).lower()
    offer = str(creator.get("package_offer", "")).lower()
    
    full_text = f"{uname} {name} {bio} {cat} {q} {offer}"
...
def main():
    print("=" * 70, flush=True)
    print("🚀 COMPLETING BATCH 5 TO EXACTLY 100 USA CREATORS (100K - 500K FOLLOWERS)")
    print("=" * 70, flush=True)
    ...
    # Load previously saved 39 creators from Batch 5 if present
            ...
            with open(OUTPUT_JSON, "r", encoding="utf-8") as jf:
                ...
                for item in saved:
                    ...
                    if u:
                        ...
                        seen.add(u)
            print(f"[*] Loaded {len(collected_creators)} verified creators already collected in Batch 5.", flush=True)
        except Exception:
            pass
    ...
    if len(collected_creators) >= TARGET_TOTAL:
        ...
        needed = TARGET_TOTAL - len(collected_creators)
        print(f"[*] Need {needed} more creators to reach {TARGET_TOTAL} total.", flush=True)

        # Build comprehensive scan URLs across expanded queries and categories
        urls_to_scan = []
        ...
        for cat in CATEGORIES:
            ...
            for pg in range(1, 30):
                ...
                urls_to_scan.append((f"https://collabstr.com/influencers?c={cat_enc}&pg={pg}", None, cat))
        ...
        with ThreadPoolExecutor(max_workers=24) as executor:
            ...
            for f in as_completed(futures):
                ...
                if res:
                    ...
                    if len(collected_creators) >= TARGET_TOTAL:
                        print(f"\n🎉 Successfully collected all {TARGET_TOTAL} brand new creators!", flush=True)
                        break
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"✨ SUCCESS: Verified exactly {len(final_100)} brand new USA creators (100k-500k followers)!", flush=True)
    ...
    # Automatically push all 100 creators to Supabase
    ...
    for i in range(0, len(clean_db_records), BATCH_SIZE):
        ...
        if upload_res.status_code in [200, 201]:
            uploaded += len(batch)
            print(f"  [✓] Upserted batch {i + 1} - {min(i + BATCH_SIZE, len(clean_db_records))} / {len(clean_db_records)} creators", flush=True)
        else:
