...
def fetch_fresh_proxies():
    ...
    for s in sources:
        try:
            ...
            pass
    print(f"[*] Scraped {len(px_set)} candidate proxies from 6 feeds.", flush=True)
    return list(px_set)
        ...
        for f in as_completed(futures):
            ...
            if res:
                ...
                if len(working) >= max_needed:
                    break
    print(f"[✓] Proxy Pool Built: {len(working)} verified live proxies ready for IP rotation!", flush=True)
    return working

async def scrape_handle_with_playwright(p, handle, proxy_str):
    ...
    try:
        ...
        await page.goto(url, wait_until="networkidle", timeout=9000)
...
async def main_async():
    print("=" * 70, flush=True)
    print("⚡ FULL CREATORDB ENRICHMENT ENGINE WITH 1-IP-PER-REQUEST ROTATION", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"[*] Pre-existing verified emails: {total_rows - len(missing_df)}", flush=True)
    print(f"[*] Target handles to scan with 1-IP-per-request Rotation: {len(missing_df)}", flush=True)
    ...
    async with async_playwright() as p:
        chunk_size = 4  # 4 concurrent requests per batch, each with a unique proxy IP
        for i in range(0, len(handles_to_check), chunk_size):
            ...
            for item_idx, (idx, h) in enumerate(chunk):
                ...
                tasks.append(scrape_handle_with_playwright(p, h, px))
            ...
            for (idx, h), (res_handle, email_found) in zip(chunk, results):
                ...
                if email_found:
                    ...
                    newly_found += 1
                    print(f"[{newly_found}] 🎯 NEW EMAIL FOUND (@{h}): {email_found} | Pushed to Supabase: {'✅' if pushed else '⚠️'}", flush=True)
            ...
            # Periodically save dataframe outputs
            if i % 20 == 0 or newly_found > 0:
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"✨ SUCCESS: Finished Full CreatorDB IP Rotated Scan!", flush=True)
