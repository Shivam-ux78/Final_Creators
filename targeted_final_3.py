...
def enrich_single_candidate(candidate: dict) -> dict:
    ...
    cand_obj = {
        ...
        "followers_num": f_num,
        "category": candidate.get("category", "Couples & Family") or "Couples & Family",
        "location": loc,
        "biography": biography,
...
def main():
    print("=" * 70, flush=True)
    print("🚀 TARGETED EXTRACTION OF FINAL 3 USA CREATORS (100K-500K)")
    print("=" * 70, flush=True)
    ...
    # Categories with exact follower range filter
    categories = [
        "Lifestyle", "Family & Children", "Travel", "Comedy & Entertainment",
        "Food & Drink", "Health & Fitness", "Fashion", "Beauty", "Art & Photography",
        "Music & Dance", "Gaming", "Outdoor & Nature", "Technology", "Home & Garden"
    ]
    ...
    for cat in categories:
        ...
        for pg in range(1, 20):
            urls_to_scan.append(f"https://collabstr.com/influencers?c={cat_enc}&min_followers=100000&max_followers=500000&loc_ids=204821&pg={pg}")
            urls_to_scan.append(f"https://collabstr.com/influencers?c={cat_enc}&min_followers=100000&max_followers=500000&pg={pg}")

    platforms = ["instagram", "tiktok", "youtube"]
    for p in platforms:
        for pg in range(1, 20):
            urls_to_scan.append(f"https://collabstr.com/influencers?platform={p}&min_followers=100000&max_followers=500000&loc_ids=204821&pg={pg}")
            urls_to_scan.append(f"https://collabstr.com/influencers?platform={p}&min_followers=100000&max_followers=500000&pg={pg}")

    print(f"[*] Scanning {len(urls_to_scan)} follower-filtered discovery pages...", flush=True)
    ...
    print(f"[✓] Discovered {len(raw_candidates)} brand new unique candidates! Verifying...", flush=True)
