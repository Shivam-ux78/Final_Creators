...
async def fetch_creatordb_page(context, handle, semaphore):
...
async def main_async():
    print("=" * 70, flush=True)
    print("⚡ PLAYWRIGHT JS-HYDRATION SCANNER: RE-CHECKING ALL REMAINING CREATORS ON CREATORDB", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"[*] Existing emails: {total_rows - len(missing_df)}", flush=True)
    print(f"[*] Target missing creators to re-check on CreatorDB: {len(missing_df)}", flush=True)
    ...
    print(f"[*] Valid Instagram handles to scan: {len(handles_to_check)}", flush=True)

    semaphore = asyncio.Semaphore(6) # 6 concurrent browser pages
    newly_found = 0
    ...
    async with async_playwright() as p:
        ...
        batch_size = 20
        for i in range(0, len(handles_to_check), batch_size):
            chunk = handles_to_check[i:i+batch_size]
            tasks = [fetch_creatordb_page(context, h, semaphore) for idx, h in chunk]
            ...
            for (idx, h), (res_handle, email_found) in zip(chunk, results):
                if email_found:
                    ...
                    df.at[idx, 'Contact Verification'] = 'Verified (CreatorDB JS Rendered)'
                    ...
                    newly_found += 1
                    print(f"[{newly_found}] FOUND ON CREATORDB: @{h} -> {email_found} | Pushed to Supabase: {'✅' if pushed else '⚠️'}", flush=True)

            # Periodically save progress
            df.to_excel(OUTPUT_EXCEL, index=False)
            df.to_csv(OUTPUT_CSV, index=False)
            print(f"[*] Processed {min(i+batch_size, len(handles_to_check))}/{len(handles_to_check)} handles | Newly found: +{newly_found}", flush=True)
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"✨ SUCCESS: Finished Playwright JS Re-check on CreatorDB!", flush=True)
