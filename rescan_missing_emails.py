...
def validate_proxy_fast(px):
    try:
        r = requests.get('https://httpbin.org/ip', proxies={'http': f'http://{px}', 'https': f'http://{px}'}, timeout=2.0)
...
def build_working_proxy_pool(max_needed=60):
    raw_list = fetch_fresh_proxies()
    working = []
    with ThreadPoolExecutor(max_workers=70) as ex:
        futures = [ex.submit(validate_proxy_fast, p) for p in raw_list[:600]]
        for f in as_completed(futures):
            ...
            if res:
                ...
                if len(working) >= max_needed:
                    break
    print(f"[✓] Proxy Pool Built: {len(working)} verified live proxies ready!", flush=True)
    return working

async def scrape_single_handle_with_retry(p, handle, proxy_pool, attempts=3):
    url = f"https://creatordb.app/creator/instagram-email-finder/?u={handle}"
    
    for attempt in range(attempts):
        # Alternate between direct request (attempt 0) and proxy IPs (attempt 1, 2)
        proxy_str = None if attempt == 0 else (proxy_pool[attempt % len(proxy_pool)] if proxy_pool else None)
        ...
        try:
            ...
            # If successfully reached page but no mail exists on page at all
            break
        except Exception:
            if browser:
                try:
...
async def main_async():
    print("=" * 70, flush=True)
    print("⚡ RESCANNING ALL MISSING CREATORS WITH MULTI-RETRY & PROXY ROTATION", flush=True)
    print("=" * 70, flush=True)

    df = pd.read_excel(OUTPUT_EXCEL)
    total_rows = len(df)
    
    missing_mask = df['Public Business Email'].isna() | (df['Public Business Email'] == '') | (df['Public Business Email'].astype(str).str.strip() == '')
    missing_df = df[missing_mask]

    print(f"[*] Total creators in spreadsheet: {total_rows}", flush=True)
    print(f"[*] Verified emails currently: {total_rows - len(missing_df)}", flush=True)
    print(f"[*] Target handles to re-scan: {len(missing_df)}", flush=True)
    ...
    proxy_pool = build_working_proxy_pool(max_needed=60)
    newly_found = 0
    scanned_count = 0

    async with async_playwright() as p:
        chunk_size = 5  # 5 concurrent requests
        for i in range(0, len(handles_to_check), chunk_size):
            chunk = handles_to_check[i:i+chunk_size]
            tasks = [scrape_single_handle_with_retry(p, h, proxy_pool) for idx, h in chunk]
            ...
            for (idx, h), (res_handle, email_found) in zip(chunk, results):
                ...
                if email_found:
                    ...
                    newly_found += 1
                    print(f"[{newly_found}] 🎯 NEW EMAIL DISCOVERED (@{h}): {email_found} | Pushed to Supabase: {'✅' if pushed else '⚠️'}", flush=True)
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"✨ SUCCESS: Finished Multi-Retry Rescan!", flush=True)
    print(f"🎉 Newly Discovered Emails: +{newly_found}")
    print(f"🏆 Total Email-Verified Creators in Dataset: {total_final} / {total_rows}")
