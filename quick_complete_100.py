...
def fetch_search_or_cat_page(task, seen_set):
    q_type, term, pg = task
    if q_type == "query":
        url = f"https://collabstr.com/influencers?q={term}&loc_ids=204821&pg={pg}"
    else:
        term_enc = term.replace(' ', '+').replace('&', '%26')
...
def main():
    ...
    if len(verified_creators) >= TARGET_COUNT:
        print(f"[✓] Goal already reached ({len(verified_creators)} creators).")
        return
    ...
    search_batches = [
        [("query", "parenting", p) for p in range(1, 15)],
        [("query", "husband+wife", p) for p in range(1, 15)],
        [("query", "couple+vlog", p) for p in range(1, 15)],
        [("query", "family+travel", p) for p in range(1, 15)],
        [("query", "raising+kids", p) for p in range(1, 15)],
        [("query", "san+francisco", p) for p in range(1, 15)],
        [("query", "nashville", p) for p in range(1, 15)],
        [("query", "austin", p) for p in range(1, 15)],
        [("category", "Family & Children", p) for p in range(1, 20)],
        [("category", "Lifestyle", p) for p in range(15, 35)]
    ]

    for batch_idx, batch_tasks in enumerate(search_batches, 1):
        if len(verified_creators) >= TARGET_COUNT:
            break
            
        print(f"[*] Batch {batch_idx}/{len(search_batches)}: crawling {len(batch_tasks)} pages...", flush=True)
        candidates = []
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(fetch_search_or_cat_page, t, seen_candidates): t for t in batch_tasks}
        ...
        print(f"  -> Discovered {len(candidates)} candidates. Enriching...", flush=True)
        if not candidates:
            continue
