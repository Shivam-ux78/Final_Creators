...
def main():
    ...
    print(f"[*] Need {TARGET_COUNT - len(verified_creators)} more creators to reach {TARGET_COUNT} target.", flush=True)
    ...
    for q in SEARCH_QUERIES:
        for pg in range(1, 25):
            tasks.append(("query", q, pg))
            
    for cat in CATEGORIES:
        for pg in range(30, 60):
            tasks.append(("category", cat, pg))

    print(f"[*] Crawling {len(tasks)} targeted pages concurrently...", flush=True)
    raw_candidates = []
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(fetch_search_or_cat_page, t, seen_candidates): t for t in tasks}
    ...
    print(f"[✓] Discovered {len(raw_candidates)} new candidate profiles. Running verification...", flush=True)
