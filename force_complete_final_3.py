...
def main():
    print("=" * 70, flush=True)
    print("🚀 FINISHING BATCH 5 TO EXACTLY 100 CREATORS")
    print("=" * 70, flush=True)
    ...
    print(f"[*] Currently have {len(collected_creators)} verified creators in batch 5.", flush=True)

    cats = [
        "Lifestyle", "Family & Children", "Travel", "Comedy & Entertainment",
        "Food & Drink", "Health & Fitness", "Fashion", "Beauty", "Art & Photography",
        "Music & Dance", "Gaming", "Outdoor & Nature"
    ]
    
    urls_to_scan = []
    for cat in cats:
        cat_enc = cat.replace(' ', '+').replace('&', '%26')
        for pg in range(15, 45):
            urls_to_scan.append(f"https://collabstr.com/influencers?c={cat_enc}&loc_ids=204821&pg={pg}")
            urls_to_scan.append(f"https://collabstr.com/influencers?c={cat_enc}&pg={pg}")

    print(f"[*] Scanning {len(urls_to_scan)} discovery pages concurrently...", flush=True)
        ...
        for f in as_completed(futures):
            ...
            if res:
                ...
                if len(collected_creators) >= TARGET_TOTAL:
                    print(f"\n🎉 REACHED EXACTLY {TARGET_TOTAL} CREATORS!", flush=True)
                    break
