...
COUPLE_KEYWORDS = [
    ...
    "home", "homestead", "life with", "adventures of"
]
...
def get_existing_usernames() -> set:
    ...
    # Also load previous merged datasets
...
def fetch_category_page(cat, pg, seen_set):
...
def main():
    ...
    # Load current output file if exists
    verified_creators = []
    seen = set()
    if os.path.exists(OUTPUT_FILE):
        try:
            ...
            print(f"[*] Resuming with {len(verified_creators)} verified creators already saved.", flush=True)
        except:
            pass

    if len(verified_creators) >= TARGET_COUNT:
        print(f"[✓] Already have {len(verified_creators)} creators in {OUTPUT_FILE}.")
        return

    print("=" * 70, flush=True)
    print("🚀 DEEP DISCOVERY & ENRICHMENT (TARGET: 100 CREATORS | 100K-300K | USA)")
    print("=" * 70, flush=True)
    ...
    for cat in CATEGORIES:
        for pg in range(1, 45):
            page_tasks.append((cat, pg))

    print(f"[*] Crawling {len(page_tasks)} category pages concurrently...", flush=True)
    with ThreadPoolExecutor(max_workers=15) as executor:
        futures = {executor.submit(fetch_category_page, cat, pg, seen_candidates): (cat, pg) for cat, pg in page_tasks}
    ...
    print(f"[✓] Discovered {len(raw_candidates)} total candidates. Starting parallel verification...", flush=True)

    with ThreadPoolExecutor(max_workers=25) as executor:
        enrich_futures = {executor.submit(enrich_single_candidate, c): c for c in raw_candidates}
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"🎉 COMPLETED: Successfully compiled {len(verified_creators)} verified Couple Creators (100k-300k | USA)")
