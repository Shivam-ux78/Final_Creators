...
def fetch_query_page(q, pg, seen_set):
...
def main():
    ...
    if os.path.exists(OUTPUT_FILE):
        try:
            ...
            print(f"[*] Loaded {len(verified_creators)} verified creators already in output file.", flush=True)
        except:
            pass
    ...
    for q in SEARCH_QUERIES:
        for pg in range(1, 30):
            page_tasks.append((q, pg))

    print(f"[*] Querying {len(page_tasks)} search pages concurrently...", flush=True)
    raw_candidates = []
    with ThreadPoolExecutor(max_workers=15) as executor:
        futures = {executor.submit(fetch_query_page, q, pg, seen_candidates): (q, pg) for q, pg in page_tasks}
