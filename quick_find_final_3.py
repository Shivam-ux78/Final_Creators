...
def enrich_single_candidate(candidate: dict) -> dict:
    ...
    cand_obj = {
        ...
        "followers_num": f_num,
        "category": candidate.get("category", "Lifestyle") or "Lifestyle",
        "location": loc,
        "biography": biography,
...
def main():
    print("=" * 70, flush=True)
    print("🚀 FINDING FINAL 3 CREATORS TO HIT EXACTLY 100 BATCH 5 CREATORS")
    print("=" * 70, flush=True)
    ...
    keywords = [
        "creator", "influencer", "model", "fitness", "vlogger", "fashion",
        "beauty", "athlete", "humor", "comedy", "artist", "dancer", "actor",
        "filmmaker", "writer", "chef", "coach", "stylist", "musician", "singer",
        "la", "nyc", "miami", "texas", "cali", "chicago", "atlanta", "seattle"
    ]
    
    urls_to_scan = []
    for kw in keywords:
        for pg in range(1, 15):
            urls_to_scan.append(f"https://collabstr.com/influencers?q={kw}&loc_ids=204821&pg={pg}")
            urls_to_scan.append(f"https://collabstr.com/influencers?q={kw}&pg={pg}")
    ...
    print(f"[✓] Found {len(raw_candidates)} brand new unique candidates! Verifying...", flush=True)
