...
OUTPUT_JSON = os.path.join("data", "extracted_100_brand_new_couples_batch3.json")
OUTPUT_CSV = os.path.join("data", "extracted_100_brand_new_couples_batch3.csv")
OUTPUT_XLSX = os.path.join("data", "extracted_100_brand_new_couples_batch3.xlsx")
...
SEARCH_QUERIES = [
    ...
    "lifestyle", "home", "homestead", "cooking+together", "family+travel",
    ...
    "oklahoma+city", "tulsa", "bakersfield", "wichita", "arlington", "new+orleans",
    "honolulu", "anaheim", "riverside", "corpus+christi", "lexington", "stockton",
    "cincinnati", "st+louis", "pittsburgh", "greensboro", "plano", "lincoln", "irvine"
]
...
COUPLE_KEYWORDS = [
    ...
    "engaged", "lovers", "hubby", "wifey", "parenting", "family time"
]
...
def enrich_single_candidate(candidate: dict) -> dict:
    ...
    try:
        ...
        if r_c.status_code == 200:
            ...
            # Check h1 for follower text if not yet known
            if not f_num:
                h1 = soup.find("h1")
                if h1:
...
def main():
    print("=" * 70, flush=True)
    print("🚀 EXTRACTING 100 BRAND NEW USA COUPLE CREATORS (0 DUPLICATES WITH DB)")
    print("=" * 70, flush=True)
            ...
            with open(OUTPUT_JSON, "r", encoding="utf-8") as jf:
                ...
                for item in saved:
                    ...
                    if u and u not in db_usernames:
                        ...
                        seen.add(u)
            print(f"Loaded {len(collected_creators)} verified brand new creators from previous run.", flush=True)
        except Exception:
            pass

    # Build search URLs across all queries and categories
    urls_to_scan = []
    ...
    with ThreadPoolExecutor(max_workers=24) as executor:
        ...
        for f in as_completed(futures):
            ...
            for c in cards:
                ...
                # Keep if not in db and within follower range or unparsed
