...
def enrich_row(index_and_row):
    ...
    if not handle:
        return idx, "", "Skipped (No handle)"

    existing_email = clean_email(str(row.get("Public Business Email", "")))
    if existing_email:
        return idx, existing_email, "Existing"

    # Step 1: Check bio text from spreadsheet
    bio_text = str(row.get("Public Bio", ""))
    if bio_text and bio_text.lower() != "nan":
        em_bio = clean_email(bio_text)
        if em_bio:
            return idx, em_bio, "Public Bio Column"

    # Step 2: Check CreatorDB API
    em_cdb = lookup_creatordb(handle)
    if em_cdb:
        return idx, em_cdb, "CreatorDB Lookup"

    # Step 3: Check Collabstr Bio & Mailto
    collab_data = lookup_collabstr_bio(handle)
    if collab_data.get("email"):
        return idx, collab_data["email"], "Collabstr Profile"

    return idx, "", "Not Found"

def main():
    print("=" * 70, flush=True)
    print("⚡ ENRICHING ALL 1,000 CREATORS IN usa_family_creators_200.xlsx", flush=True)
    print("=" * 70, flush=True)

    df = pd.read_excel(INPUT_EXCEL, header=6)
    total_rows = len(df)
    
    initial_emails = df['Public Business Email'].dropna().apply(lambda x: clean_email(str(x)))
    initial_count = len([e for e in initial_emails if e])
    print(f"[*] Initial verified emails in spreadsheet: {initial_count}/{total_rows}", flush=True)
    print(f"[*] Checking remaining {total_rows - initial_count} creators via CreatorDB + Bio Scraping...", flush=True)
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = [executor.submit(enrich_row, item) for item in rows_to_process]
        for f in as_completed(futures):
            idx, email_found, source = f.result()
            if email_found:
                with lock:
                    ...
                    r = df.iloc[idx]
                    handle = str(r.get("Instagram Handle", "")).strip().lstrip("@")
                    ...
                    newly_found += 1
                    total_current = initial_count + newly_found
                    print(f"[{total_current}/{total_rows}] NEW EMAIL FOUND ({source}): @{handle} | {email_found} | Pushed to Supabase: {'✅' if pushed else '⚠️'}", flush=True)

    # Save outputs
    df.to_excel(OUTPUT_EXCEL, index=False)
    df.to_csv(OUTPUT_CSV, index=False)
    
    # Save JSON of email-verified records
    ...
    final_email_count = len(df['Public Business Email'].dropna())
    live_cnt = get_live_db_count()

    print("\n" + "=" * 70, flush=True)
    print(f"✨ SUCCESS: Finished CreatorDB & Bio Email Enrichment!", flush=True)
    print(f"📊 Initial Emails: {initial_count}")
    print(f"🎉 Newly Discovered Emails: +{newly_found}")
    print(f"🏆 Total Email-Verified Creators: {final_email_count} / {total_rows}")
    print(f"🚀 Total Creators Live in Supabase: {live_cnt}")
