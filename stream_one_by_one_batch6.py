...
OUTPUT_JSON = os.path.join("data", "extracted_100_brand_new_100k_500k_batch6.json")
OUTPUT_CSV = os.path.join("data", "extracted_100_brand_new_100k_500k_batch6.csv")
OUTPUT_XLSX = os.path.join("data", "extracted_100_brand_new_100k_500k_batch6.xlsx")
...
EXPANDED_QUERIES = [
    ...
    "family", "parents", "duo", "mom", "dad", "parenting", "motherhood",
    "california", "florida", "texas", "new+york", "los+angeles", "miami",
    "austin", "dallas", "chicago", "atlanta", "phoenix", "seattle", "denver", "nashville",
    "lifestyle", "fitness", "travel", "foodie", "fashion", "beauty", "comedy", "homestead"
]
...
def enrich_single_candidate(candidate: dict) -> dict:
    s = get_session()
    username = candidate.get("username", "").strip().lower().lstrip("@")
    if not username:
        return None
    ...
    try:
        r_c = s.get(f"https://collabstr.com/{username}", timeout=4)
...
def main():
    print("=" * 70, flush=True)
    print("⚡ STREAMING ONE-BY-ONE IMMEDIATE PUSH ENGINE (100K-500K FOLLOWERS | USA)", flush=True)
    print("=" * 70, flush=True)
            ...
            with open(OUTPUT_JSON, "r", encoding="utf-8") as jf:
                ...
                for item in saved:
                    ...
                    if u and em and u not in [c['username'] for c in collected_creators]:
                        ...
                        seen.add(u)
            print(f"[*] Loaded {len(collected_creators)} verified creators already collected in Batch 6 file.", flush=True)
        except Exception:
            pass
    ...
    if len(collected_creators) >= TARGET_BATCH_COUNT:
        print(f"[✓] Already have {len(collected_creators)} creators collected in Batch 6!", flush=True)
        return

    needed = TARGET_BATCH_COUNT - len(collected_creators)
    print(f"[*] Target: {len(collected_creators)}/100 collected. Real-time extraction & IMMEDIATE PUSH for remaining {needed} creators...", flush=True)

    urls_queue = queue.Queue()
    for q in EXPANDED_QUERIES:
        for pg in range(1, 15):
            urls_queue.put((f"https://collabstr.com/influencers?q={q}&loc_ids=204821&pg={pg}", q, None))
            urls_queue.put((f"https://collabstr.com/influencers?q={q}&pg={pg}", q, None))
    for cat in CATEGORIES:
        cat_enc = cat.replace(' ', '+').replace('&', '%26')
        for pg in range(1, 18):
            urls_queue.put((f"https://collabstr.com/influencers?c={cat_enc}&loc_ids=204821&pg={pg}", None, cat))
            urls_queue.put((f"https://collabstr.com/influencers?c={cat_enc}&pg={pg}", None, cat))
    ...
    def scanner_worker():
        s = get_session()
        while not stop_event.is_set() and not urls_queue.empty():
            try:
                url_item = urls_queue.get_nowait()
            except queue.Empty:
                break
            url, q, cat = url_item
            try:
                ...
                if r.status_code == 200:
                    ...
                    for el in elements:
                        ...
                        if parsed and parsed.get("username"):
                            ...
                            u = str(parsed["username"]).strip().lower().lstrip("@")
                            f_num = parsed.get("followers_num", 0)
                            if not f_num and parsed.get("followers"):
                                f_num = parse_num(parsed.get("followers"))
                                parsed["followers_num"] = f_num

                            with file_lock:
                                if u and u not in seen:
                                    if f_num == 0 or (MIN_FOLLOWERS <= f_num <= MAX_FOLLOWERS):
                                        seen.add(u)
                                        candidate_queue.put(parsed)
            except Exception:
                pass
            urls_queue.task_done()

    def verifier_worker():
        nonlocal collected_creators
        while not stop_event.is_set():
            try:
                candidate = candidate_queue.get(timeout=1)
            except queue.Empty:
                if urls_queue.empty() and candidate_queue.empty():
                    break
                continue

            res = enrich_single_candidate(candidate)
            if res:
                with file_lock:
    ...
    print("[*] Launching parallel scanner & verifier workers...", flush=True)
    threads = []
    for _ in range(12):
    ...
    for _ in range(16):
        t = threading.Thread(target=verifier_worker)
        t.start()
        threads.append(t)
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"Complete & push all 100 Batch 6 creators to Supabase DB (Live count: {live_total})"], check=False)
        subprocess.run(["git", "push", "origin", "main"], check=False)
        print("[✓] Pushed updated Batch 6 files to git origin/main.", flush=True)
    except Exception as e:
